...
public class CityHash {
	...
	private static long fetch64(byte[] byteArray, int start) {
		return ByteUtils.bytesToLong(byteArray, start, ByteUtils.CPU_ENDIAN);
	}

	private static int fetch32(byte[] byteArray, final int start) {
		return ByteUtils.bytesToInt(byteArray, start, ByteUtils.CPU_ENDIAN);
	}
